...
reifyUnit (Unit _ s)   = [t| MkUoM $(litT (strTyLit s))            |]
...
-- and create corresponding 'MkUoM' type instance declarations.
uDec :: String -> Q [Dec]
...
-- appropriate instance of the 'MkUoM' type family.
declareUnit :: String -> UnitDecl -> Q [Dec]
declareUnit s ud = case ud of
  BaseUnit ->
      [d|
          type instance MkUoM $(litT (strTyLit s)) = Base $(litT (strTyLit s)) 1
          instance
              ( IsCanonical (Unpack (Base $(litT (strTyLit s)) 1))
              , (Base $(litT (strTyLit s)) 1 /: Base $(litT (strTyLit s)) 1) ~ One
              ) => HasCanonicalBaseUnit $(litT (strTyLit s))
      |]

  DefinedUnit u ->
      [d|
          type instance MkUoM $(litT (strTyLit s)) = $(reifyUnit u)
      |]
  ...
  ConversionUnit r u ->
      [d|
          type instance MkUoM $(litT (strTyLit s)) = Base $(litT (strTyLit s)) 1
          instance IsCanonical (Unpack $(reifyUnit u)) => HasCanonicalBaseUnit $(litT (strTyLit s)) where
...
-- produces
--
-- > type instance MkUoM "N" = Base "kg" *: Base "m" /: Base "s" ^: 2
--
...
-- produces
--
-- > type instance MkUoM "kilobyte" = Base "kilobyte"
