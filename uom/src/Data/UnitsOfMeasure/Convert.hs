...
class IsCanonical (Unpack (CanonicalBaseUnit s))
    => HasCanonicalBaseUnit (s :: Symbol) where
  ...
  type CanonicalBaseUnit s :: UoM
  type CanonicalBaseUnit s = Base s 1

  -- | The conversion ratio between this base unit and its canonical
  -- base unit.  If @b@ is canonical then this ratio is @1@.
  conversionBase :: n ~ 1 => proxy s -> Quantity Rational (Base s n /: CanonicalBaseUnit s)

  default conversionBase
      :: (n ~ 1, Base s n ~ CanonicalBaseUnit s, (CanonicalBaseUnit s /: CanonicalBaseUnit s) ~ One)
      => proxy s
      -> Quantity Rational (Base s n /: CanonicalBaseUnit s)
  conversionBase _ = 1

-- | Convert a unit into its canonical representation, where units are
-- represented syntactically.
type family ToCBU (u :: UnitSyntax Symbol) :: UoM where
  ToCBU (xs ':/ ys) = ListToCBU xs /: ListToCBU ys

type family ListToCBU (xs :: [Symbol]) :: UoM where
  ListToCBU '[]       = One
...
type family AllIsCanonical (xs :: [Symbol]) :: Constraint where
  AllIsCanonical '[] = ()
  AllIsCanonical (x ': xs) = (CanonicalBaseUnit x ~ Base x 1, AllIsCanonical xs)
...
help
    ...
    . HasCanonical u
    => SUnit u
    -> Quantity Rational (Pack u /: ToCBU u)
...
help'
    ...
    . AllHasCanonical xs
    => SList xs
    -> Quantity Rational (Prod xs /: ListToCBU xs)
help' SNil = 1
...
-- @'ratio' [u| ft |] [u| m |]@.
ratio :: forall a u v (proxy :: UoM -> *) proxy' .
