...
module ThoralfPlugin.Encode.Nat
  ( natTheory,
  )
where
...
import Type (TyVar, Type, isNumLitTy, splitTyConApp_maybe, tyVarKind)
...
natAddConv ty = do
  ...
  case (tycon == typeNatAddTyCon, types) of
    (True, [x, y]) ->
      let mkNatSExpr :: Vec Two String -> Vec 'Zero String -> String
          mkNatSExpr (a :> b :> VNil) VNil = "(+ " ++ a ++ " " ++ b ++ ")"
          tyList = x :> y :> VNil
       in return $ TyConvCont tyList VNil mkNatSExpr []
    (_, _) -> Nothing
